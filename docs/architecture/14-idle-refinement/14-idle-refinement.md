...
| 9 | 文档维护（Step 9，2026-08-08） | 规则 | 文档引用链接对齐（决策 43 §4.1）：9.1 相对链接 / 9.2 INDEX 缺行 / 9.3 changelog 校验+draft / 9.4 trace 映射 / 9.5 viking:// URI / 9.6 wiki 残留 / 9.7 OV 侧对齐 / 9.8 代码注释链接。**零 LLM 零 token**，默认 dry-run 只报告（`REFINEMENT_DOC_DRY_RUN`），apply 需显式配置 |
...
| `REFINEMENT_DOC_MAINTENANCE` | False | Step 9 文档维护总开关（默认关，同其他 Step） |
| `REFINEMENT_DOC_DRY_RUN` | True | Step 9 默认只报告不落盘（安全默认，人工确认后 apply） |
| `REFINEMENT_DOC_OV_APPLY` | False | Step 9 OV 侧落盘开关（权威源默认只报告） |

详见：[34-空闲精炼管线（v2 reality 化）](../../decisions/34-idle-refinement/34-idle-refinement.md)
