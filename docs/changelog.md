...
| **v6.4** | 2026-07-31 | Strand 多事务摘要重构（已实现 → v6.4.1） |
| **v7** | 2026-08-07 | **生产 reality 化（决策 37-41，theme 退役）** | **reality 重构（37）**：reality 取代 theme，OODA 与 OV L1 对齐，宁分不并。**图模型（38）**：共现边（PMI 距离）+ S 匹配分替代余弦，注入 4B 拣选，FAR 切换迁移 4B。**成员云（39）**：提问云/工作云同域匹配。**flash 重跑 pilot（40）**：独立 DB 零污染生产，111 reality 精炼。**生产 reality 化（41，2026-08-07 落地）**：realities 表迁移（111 行 + s2r 574，块键映射 + centroid 语义匹配 cos≥0.55）；注入 = 提问云形心散度距离（d=1-cos，θ_max=0.5，top-15 → 4B top-3，jaccard 兜底）；归并 run_reality_merge（S 匹配分 + 防膨胀守卫 + query_centroid 增量）；graphify 节点 reality_{id}（sync_realities_to_graph + cooc reality 边）；refinement/wiki_to_graph 全链路 realities；themes/theme_strand_map 冻结归档；全量 pytest 787 passed。**验证**：graph.json 重建 111 reality 节点 + 574 merged_into 边（节点数=表行数对齐）；winker/sysadmin 代码同步（sync-ca.sh）。 | **已落地** |
 以 strand（事务/工作线）取代话题块作为摘要单元：一个话题块内 4B 识别 2-6 个 strand，各有 hdl/turns/独立 OODA 四组；wiki 重构（strand 为 source 颗粒，不向前兼容）；A-stage 沿用话题块 grade 不动；宁多勿少 + 识别失败降级单 strand。详见 35-strand-multi-affair-summarization.md + `.hermes/plans/2026-07-31_ca-strand-refactor.md`。→ **已随 v6.4.1 实装**。 | **已实装** |
