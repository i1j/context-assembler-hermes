...
| **BUG-08** | `__init__.py:1011-1014` + inject 4B | 用户消息路径同步阻塞：turn==1 `while + time.sleep(0.5)` 等清账 5s；`pick_injection_realities` 内同步 4B 调用超时 `LLM_TIMEOUT=120s` × `LLM_MAX_RETRIES=2` + 退避 → 首轮/FAR 切换用户消息可被阻塞分钟级 | ✅ 代码确认 |
