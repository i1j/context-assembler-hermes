...
```python
def _on_pre_llm_call_v5(self, ...):
    write_origin = get_current_write_origin()
    if write_origin == "skill_provenance":
        # bg_review 轮：跳过 A-stage，直接写 Fct
        self.store.write_fct_hdl(turn, fct=user_content, hdl="")
        return  # 跳过后续装配
    # 正常装配路径...
```

**实现要点**：
- **检测方式**：`get_current_write_origin()` 返回 `'skill_provenance'` 时判定为 bg_review
- **跳过 A-stage 组装**：不执行角色队列匹配和 grade 替换
- **同步写 Fct**：把 user 消息原文直接填入 Fct 列
- **不写 Hdl**：bg_review 不产生历元摘要
...
- bg_review 路径现在有独立测试覆盖（`TestBgReview.test_bg_review_writes_fct_equals_content`）
- bg_review 的 Fct 值实际是原文，不是摘要——消费方需知晓此语义差异
