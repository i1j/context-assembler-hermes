...
- R5.3 Fct 输出改为**多事务 OODA JSON**：`{"affairs":[{"hdl","turns","ooda":{"现象与问题","背景与约束","决策与方案","后续行动"},"changes":[{"stage_tag","core_change"}]}]}`；代码解析校验后**扁平化**为 legacy `changes/core_change/四段字段`，`affairs` 一并存入 Fct，topic_summary/strand/A-stage 既有消费链不破。
  - **已被决策 45 升级**：v3 记录去掉 `changes`/`stage_tag`，OODA 阶段项即变更（`_fct_format = "v3-multi-affair-ooda"`）；v2 仅旧库只读兼容。v1.1 起 `flatten_affairs_to_legacy` 已删除，改 `build_fct_multi_affair` 只写 `affairs + 装配元数据`，旧消费者视图由 `collect_turn_fcts` 读路径现场派生。
