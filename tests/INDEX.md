...
1. **Hermetic 环境** — 跑测试建议用 `python -m pytest tests/ -q -n 4`（CI 环境已隔离 API key + 临时 HOME + UTC）。`conftest.py` 在根层级仅做 sys.path 配置 + re-export fixtures；autouse mock 按需下沉到 `stage/conftest.py` 和 `plugin/conftest.py`，不污染不需要 mock 的测试。
...
| 归档 | （无独立目录） | v4.6 旧 API 测试已删除（标记 `@pytest.mark.v460` 已移除） |
...
| 决策类型 | 测试文件 |
|---------|---------|
| 话题拣选 (TP-*) | test_topic_manager, test_grade, test_a_stage_topic_aware, test_embedding, test_cross_ref_wiki_audit |
| A-stage (C-010, V-*) | test_a_stage, test_a_stage_topic_aware |
| E-stage (SC-*) | test_e_stage, test_store_v5, test_embedding |
| F-stage (R-004, L1-*) | test_f_stage, test_parse_v1 |
| 存储层 (S-*) | test_store_v5 |
...
- 单元测试: 9 文件（test_topic_manager、test_grade、test_cache、test_retrieval、test_stats、test_tool_summarizer、test_ce_shell、test_embedding、test_lstage）
- 集成测试: 5 文件（test_a_stage、test_a_stage_topic_aware、test_e_stage、test_f_stage、test_parse_v1）
- 存储测试: 3 文件（test_store_v5、test_store_contract、test_embedding）
- 插件测试: 1 文件（test_plugin）
- 配置测试: 2 文件（test_config、test_health）
- 审计测试: 3 文件（test_cross_ref_wiki_audit、test_debug_dump_audit、test_doa_self_defense）
